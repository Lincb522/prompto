...
export function HistoryDrawer({ open, onClose }: HistoryDrawerProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 right-0 bottom-0 w-[360px] max-w-[85vw] z-50",
          "glass-card rounded-none rounded-l-lg",
          "flex flex-col animate-slide-in-right"
        )}
      >
        {/* 头部 */}
        <div className="flex items-center justify-between px-4 h-14 border-b border-white/20 dark:border-white/10">
          <span className="text-sm font-semibold whitespace-nowrap">记录</span>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          ...
          {pinned.length > 0 && (
            <>
              <p className="text-xs font-medium text-fg-muted dark:text-fg-dark-muted px-1 whitespace-nowrap">
                已固定
              </p>
              {pinned.map((item) => (
                <HistoryCard
          ...
          {unpinned.length > 0 && pinned.length > 0 && (
            <p className="text-xs font-medium text-fg-muted dark:text-fg-dark-muted px-1 pt-2 whitespace-nowrap">
              全部
            </p>
          )}
          {unpinned.map((item) => (
  ...
  return (
    <div className="group rounded-lg border border-white/20 dark:border-white/10 p-3 hover:border-primary/30 dark:hover:border-primary-light/30 transition-colors duration-200 cursor-pointer">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button
            ...
            className={cn(
              "btn-icon !w-6 !h-6",
              item.pinned && "text-primary dark:text-primary-light"
            )}
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded bg-primary/10 text-primary dark:text-primary-light whitespace-nowrap">
          {item.engine}
        </span>
      </div>
    </div>
