import { ArrowRight, Clipboard, Wand2, Clock, Zap, Circle, Layers, BarChart3, Sparkles } from "lucide-react";
import { useAppStore, type McpStatus } from "@/store/app-store";
...
export function HomePage({ onNavigate }: Props) {
  ...
  const mcpCount = mcpInstalled.length;

  const startEmpty = () => onNavigate("editor");
  const startFromClipboard = async () => {
    try {
      const text = await api.readClipboard();
      if (text && (text as string).trim()) setInput(text as string);
    } catch {}
    onNavigate("editor");
  };
      ...
      <div className="max-w-4xl mx-auto px-6 py-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="glass-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-fg dark:text-fg-dark">引擎</span>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {config.engine === "cli-passthrough" && (
                <span className="text-[11px] text-fg-muted dark:text-fg-dark-muted">
                  → {config.target_cli}
                </span>
              )}
            </div>
            <div className="flex items-center gap-3">
              {clis.map((c) => (
                <div key={c.cli} className="flex items-center gap-1">
                  ...
                  <span className="text-[11px] text-fg-muted dark:text-fg-dark-muted">{c.cli}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold text-fg dark:text-fg-dark">MCP</span>
              <span className={cn("text-[10px] px-1.5 py-0.5 rounded-md font-medium", mcpCount > 0 ? "bg-primary/10 text-primary" : "bg-white/30 dark:bg-white/10 text-fg-muted dark:text-fg-dark-muted")}>
            ...
            {mcpCount > 0 ? (
              <div className="flex items-center gap-1.5 flex-wrap">
                {mcpInstalled.map((s) => (
                  <span key={s.ide} className="text-[10px] px-1.5 py-0.5 rounded bg-white/40 dark:bg-white/10 text-fg-muted dark:text-fg-dark-muted">
                    {s.ide}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-fg dark:text-fg-dark">最近</span>
              </div>
              <button
                onClick={() => { if (recent[0]) { loadFromHistory(recent[0]); onNavigate("editor"); } }}
                className="text-[11px] text-primary hover:text-primary-hover cursor-pointer flex items-center gap-1"
              >
        ...
        <div className="pt-2 text-center">
          <span className="text-[10px] text-fg-muted/40 dark:text-fg-dark-muted/40">
            Prompto v0.2.0 · VS Code Extension
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/40 dark:bg-white/10">
      ...
      <div className="flex flex-col">
        <span className="text-[10px] text-fg-muted dark:text-fg-dark-muted">{label}</span>
        <span className={cn("text-sm font-bold", color === "primary" ? "text-primary" : "text-fg dark:text-fg-dark")}>{value}</span>
      </div>
    </div>
  );
}
...
function ActionCard({ icon, title, desc, color, onClick, disabled }: {
  ...
  return (
    ...
    >
      ...
      <div className="text-sm font-semibold text-fg dark:text-fg-dark">{title}</div>
      <div className="text-[11px] text-fg-muted dark:text-fg-dark-muted mt-0.5">{desc}</div>
    </button>
  );
}
