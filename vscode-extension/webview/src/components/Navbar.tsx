...
export function Navbar({ page, onNavigate, onOpenHistory, onOpenSettings }: NavbarProps) {
  ...
  const currentTheme = (config?.theme ?? "light") as string;
  ...
  return (
    <header className="h-11 flex items-center px-3 glass-card rounded-none border-x-0 border-t-0 relative z-50 select-none shrink-0">
      {/* 中间导航 */}
      <div className="flex-1 flex items-center justify-center gap-1">
        ...
        <div className="flex items-center gap-0.5 p-0.5 rounded-lg bg-white/30 dark:bg-white/10">
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium cursor-pointer transition-all duration-150",
              page === "home"
        ...
        {page === "editor" && (
          <div className="flex items-center gap-1.5 ml-3">
            <Dropdown
              ...
              onChange={(v) => updateConfig({ engine: v as any })}
            />
            {config?.engine === "cli-passthrough" && (
              <Dropdown
                options={CLI_OPTIONS}
                value={config?.target_cli ?? "codex"}
                onChange={(v) => updateConfig({ target_cli: v as any })}
              />
            )}
          </div>
        )}
